        ...
        for node in node_tree.nodes:
            if node.type == "BSDF_PRINCIPLED":
                for input in node.inputs:
                    ...
                    # Special handling for vector inputs that only need to check if linked
                    if socket_name in ("Normal", "Tangent", "Clearcoat Normal"):
                        if input.is_linked:
    ...
    def invoke(self, context, event):
        ...
        for obj in objects:
            ...
            for slot in obj.material_slots:
                ...
                # Process node tree with appropriate method based on shift key
