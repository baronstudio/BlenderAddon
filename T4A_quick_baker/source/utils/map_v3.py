...
class Map:
        ...
        for node in node_tree.nodes:
            if node.type == "BSDF_PRINCIPLED":
                if node.inputs["Emission"].is_linked:
                    self.NODE_DATA[node_tree]["Emission Socket"] = node.inputs["Emission"].links[0].from_socket
            ...
            for node in node_tree.nodes:
                if node.type == "BSDF_PRINCIPLED":
                    for key, value in values.items():
                        if key == "Emission Socket":
                            node_tree.links.new(output=value, input=node.inputs["Emission"])
            ...
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    ...
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_value_nodes(node_tree, input="Specular")
            ...
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    ...
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_value_nodes(node_tree, input="Subsurface")
            ...
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    ...
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_vector_nodes(node_tree, input="Subsurface Radius")

    # Subsurface Color
    def prepare_subsurface_color(self):
        """Prepare Subsurface Color"""
        for obj in self.context.selected_objects:
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    node_tree = slot.material.node_tree
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_color_nodes(node_tree, input="Subsurface Color")
            ...
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    ...
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_value_nodes(node_tree, input="Subsurface IOR")
            ...
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    ...
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_value_nodes(node_tree, input="Subsurface Anisotropy")
            ...
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    ...
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_value_nodes(node_tree, input="Specular Tint")
            ...
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    ...
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_value_nodes(node_tree, input="Sheen")
            ...
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    ...
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_value_nodes(node_tree, input="Sheen Tint")

    # Clearcoat
    def prepare_clearcoat(self):
        """Prepare Clearcoat"""
        for obj in self.context.selected_objects:
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    ...
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_value_nodes(node_tree, input="Clearcoat")

    # Clearcoat Roughness
    def prepare_clearcoat_roughness(self):
        """Prepare Clearcoat Roughness"""
        for obj in self.context.selected_objects:
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    node_tree = slot.material.node_tree
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_value_nodes(node_tree, input="Clearcoat Roughness")
    ...
    def prepare_transmission_roughness(self):
        """Prepare Transmission Roughness"""
        for obj in self.context.selected_objects:
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    node_tree = slot.material.node_tree
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_value_nodes(node_tree, input="Transmission Roughness")
    ...
    # Clearcoat Normal
    def prepare_clearcoat_normal(self):
        """Prepare Clearcoat Normal"""
        for obj in self.context.selected_objects:
            for slot in obj.material_slots:
                if slot.material and slot.material.use_nodes:
                    node_tree = slot.material.node_tree
                    if node_tree in self.NODE_DATA:
                        continue
                    self.remove_nodes(node_tree=node_tree)
                    self.prepare_vector_nodes(node_tree, input="Clearcoat Normal")
    ...
    def prepare_diffuse_nodes(self, node_tree):
        ...
        for node in node_tree.nodes:
            if node.type == "BSDF_PRINCIPLED":
                # Store links
                if node.inputs["Subsurface"].is_linked:
                    self.NODE_DATA[node_tree]["Subsurface Socket"] = node.inputs["Subsurface"].links[0].from_socket
                    node_tree.links.remove(node.inputs["Subsurface"].links[0])
                ...
                if node.inputs["Sheen"].is_linked:
                    self.NODE_DATA[node_tree]["Sheen Socket"] = node.inputs["Sheen"].links[0].from_socket
                    node_tree.links.remove(node.inputs["Sheen"].links[0])
                if node.inputs["Transmission"].is_linked:
                    self.NODE_DATA[node_tree]["Transmission Socket"] = node.inputs["Transmission"].links[0].from_socket
                    node_tree.links.remove(node.inputs["Transmission"].links[0])

                # Store default values
                self.NODE_DATA[node_tree]["Subsurface Value"] = node.inputs["Subsurface"].default_value
                self.NODE_DATA[node_tree]["Metallic Value"] = node.inputs["Metallic"].default_value
                self.NODE_DATA[node_tree]["Sheen Value"] = node.inputs["Sheen"].default_value
                self.NODE_DATA[node_tree]["Transmission Value"] = node.inputs["Transmission"].default_value
        ...
        for node_tree, values in self.NODE_DATA.items():
            for node in node_tree.nodes:
                if node.type == "BSDF_PRINCIPLED":
                    for key, value in values.items():
                        # Restore links
                        if key == "Subsurface Socket":
                            node_tree.links.new(output=value, input=node.inputs["Subsurface"])
                        if key == "Metallic Socket":
                            node_tree.links.new(output=value, input=node.inputs["Metallic"])
                        if key == "Sheen Socket":
                            node_tree.links.new(output=value, input=node.inputs["Sheen"])
                        if key == "Transmission Socket":
                            node_tree.links.new(output=value, input=node.inputs["Transmission"])
                        # Restore default values
                        if key == "Subsurface Value":
                            node.inputs["Subsurface"].default_value = value
                        if key == "Metallic Value":
                            node.inputs["Metallic"].default_value = value
                        if key == "Sheen Value":
                            node.inputs["Sheen"].default_value = value
